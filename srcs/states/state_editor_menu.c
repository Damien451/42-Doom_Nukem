...
/*   Updated: 2020/04/19 18:29:23 by damien           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	buttons_editor_menu(t_doom *data, t_button buttons[4],
	char map_name[25])
{
	buttons[0] = button(point(960 - (274 * 3 / 2), 540 - (90 + 30)),
		point(274 * 3, 90), EDITOR, map_name);
...
int			state_editor_menu(t_doom *data)
{
	...
	if (!first)
	{
		...
		if (map_name[0] == '\0')
			switch_state(data, data->state, LEAVING);
		data->map_name = map_name;
	}
	buttons_editor_menu(data, buttons, map_name);
