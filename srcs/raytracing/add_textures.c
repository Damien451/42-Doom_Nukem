...
unsigned int				add_skybox(t_vec3d intersect, const t_doom *const data)
{
	if (intersect.x == 64.0)
		return ((fill_percent_512((64.0 - intersect.y) / 64.0
					, intersect.z / 64.0, data->skybox[0]) & 0xFEFEFE) >> 1);
	else if (intersect.x == 0.0)
		return ((fill_percent_512((64.0 - intersect.y) / 64.0
					, (64.0 - intersect.z) / 64.0, data->skybox[2]) & 0xFEFEFE) >> 1);
	else if (intersect.y == 64.0)
		return ((fill_percent_512(intersect.x / 64.0
					, intersect.z / 64.0, data->skybox[4]) & 0xFEFEFE) >> 1);
	else if (intersect.y == 0.0)
		return ((fill_percent_512(intersect.x / 64.0
					, intersect.z / 64.0, data->skybox[5]) & 0xFEFEFE) >> 1);
	else if (intersect.z == 64.0)
	{
		return ((fill_percent_512((64.0 - intersect.y) / 64.0
					, (64.0 - intersect.x) / 64.0, data->skybox[1]) & 0xFEFEFE) >> 1);
	}
	return ((fill_percent_512((64.0 - intersect.y) / 64.0
				, intersect.x / 64.0, data->skybox[3]) & 0xFEFEFE) >> 1);
}
